...
from module.rl_module import CFAPredictor
...
class SAR(AUXBase):
    ...
    def __init__(self, action_shape, extr_latent_dim, nstep_of_rsd, hidden_dim,
        ...
        # Initialize modules
        self.network = CFAPredictor(extr_latent_dim,
                                   act_seq_in_dim,
                                   nstep_of_rsd,
                                   hidden_dim,
                                    act_seq_out_dim,
    ...
    def calc_psi(self, a_seq, w_seq):
        if self.rs_fc:
            a_seq = self.network.forward_as(a_seq) #
        inner_product = w_seq @ a_seq.t() #
    ...
    def update_extr(self, data, aug_s, num_aug):
        ...
        psi_targ = self.calc_psi(a_seq, w_seq)  # (num_sample, batch_size*num_aug, 2)
        
        psi_cos, psi_sin = self.network(aug_s, r_seq, w_seq).chunk(2, -1) 
